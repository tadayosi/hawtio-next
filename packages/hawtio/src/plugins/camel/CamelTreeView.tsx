import { MBeanNode, MBeanTree, PluginTreeViewToolbar, workspace, ExpansionValue } from '@hawtiosrc/plugins/shared'
...
export const CamelTreeView: React.FunctionComponent = () => {
  const { tree, selectedNode, setSelectedNode } = useCamelContext()
  const [allExpanded, setAllExpanded] = useState(workspace.allExpanded(jmxDomain))
  ...
  const onAllExpanded = (expanded: boolean) => {
    workspace.expandAll(expanded, jmxDomain)
